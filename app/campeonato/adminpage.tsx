...
export default function Campeonatos() {
    ...
    function getTimesPorGrupo(grupo: string) {
        return times.filter(t =>
            t.competicoes.some(c =>
                c.nome === "Copa Verde" && c.grupo === grupo
            )
        );
    }

    function gerarTabelaCopaVerde(grupo: string) {
        const tabela: any = {};

        const timesGrupo = getTimesPorGrupo(grupo);

        // 🔥 usar NOME como chave (igual jogos)
        ...
        jogos
            .filter(j => j.competicao === "Copa Verde")
            .forEach(j => {
                const casa = tabela[j.casa];
                const fora = tabela[j.fora];
                    ...
                    <div className="bg-white p-2 rounded-xl shadow">
                        ...
                        {tabela.map((t: any, i) => {
                            const timeInfo = times.find(
                                (time) =>
                                    time.nome.toLowerCase().trim() === t.time.toLowerCase().trim()
                            );
                        ...
                        {Object.keys(rodadas).map((r) => (
                            <div key={r} className="mb-4">
                                ...
                                {rodadas[r].map((j: Jogo, i: number) => {
                                    const casa = times.find(
                                        (t) => t.nome.toLowerCase().trim() === j.casa.toLowerCase().trim()
                                    );

                                    const fora = times.find(
                                        (t) => t.nome.toLowerCase().trim() === j.fora.toLowerCase().trim()
                                    );
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div className="text-xs text-gray-700">
                                                {new Date(j.dataInput).toLocaleString("pt-BR")}
            ...
            {ativo === "Copa Verde" && (
                <div className="grid gap-6">
                    ...
                    <div className="bg-white p-4 rounded-xl shadow">
                        ...
                        <div className="grid grid-cols-5 font-bold border-b pb-2 text-black text-sm">
                            <span>#</span>
                            <span className="col-span-2">Time</span>
                            <span>Pts</span>
                            <span>SG</span>
                        ...
                        {gerarTabelaCopaVerde("A").map((t: any, i) => {
                            const timeInfo = times.find(
                                (time) =>
                                    time.nome.toLowerCase().trim() === t.time.toLowerCase().trim()
                            );
                            ...
                            return (
                                <div
                                    key={i}
                                    className="grid grid-cols-5 items-center p-2 border-b text-black text-sm"
                                >
                                    <span>{i + 1}</span>
                    ...
                    <div className="bg-white p-4 rounded-xl shadow">
                        ...
                        {gerarTabelaCopaVerde("B").map((t: any, i) => {
                            const timeInfo = times.find(
                                (time) =>
                                    time.nome.toLowerCase().trim() === t.time.toLowerCase().trim()
                            );
